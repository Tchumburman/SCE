...
welcome_ui <- nav_panel_hidden(
  ...
  div(
    ...
    tags$hr(),
    # --- "Don't know where to start?" collapsible card ---
    card(
      class = "border-warning mb-4 nav-card-hover",
      style = "cursor: pointer;",
      onclick = "var body = this.querySelector('.start-suggestions'); var chevron = this.querySelector('.start-chevron'); if(body){ body.classList.toggle('d-none'); chevron.classList.toggle('fa-chevron-down'); chevron.classList.toggle('fa-chevron-up'); }",
      card_body(
        class = "py-3",
        div(
          class = "d-flex align-items-center gap-3",
          div(icon("compass", class = "fa-2x text-warning")),
          div(
            tags$h5(class = "mb-1", "Don\u2019t know where to start?"),
            tags$p(class = "mb-0 text-muted",
              "Click to see a suggested learning path for beginners.")
          ),
          div(class = "ms-auto", tags$i(class = "fa fa-chevron-down start-chevron text-warning fa-lg"))
        ),
        tags$div(
          class = "start-suggestions d-none mt-3",
          tags$p(class = "text-muted mb-2", style = "font-size: 0.85rem;",
            "Try these topics in order \u2014 they build on each other and cover the essentials:"),
          tags$div(
            ...
            lapply(
              ...
              function(topic) {
                ...
                d     <- if (!is.null(meta)) meta$difficulty else 1
                d_col <- c("#859900", "#2aa198", "#b58900", "#cb4b16", "#dc322f")[d]
                dots  <- paste(vapply(seq_len(5), function(i)
                  if (i <= d) "\u25CF" else "\u25CB", character(1)), collapse = "")
                tags$span(
                  ...
                  icon("arrow-right", class = "me-1",
                       style = "font-size: 0.65rem; opacity: 0.5;"),
                  topic,
                  tags$span(
                    style = sprintf("margin-left: 0.4rem; color: %s; font-size: 0.65rem; letter-spacing: 1px;", d_col),
                    dots
                  )
                )
              }
...
server <- function(input, output, session) {
  ...
  observeEvent(input$main_nav, {
    ...
    # Inject topic header (difficulty + related) at top of the page
    header_ui <- topic_page_header(tab)
    if (!is.null(header_ui)) {
      # bslib tab panes use data-value attribute
      selector <- sprintf(".tab-pane[data-value='%s']", gsub("'", "\\\\'", tab))
      insertUI(
        selector = selector,
        where    = "afterBegin",
        ui       = header_ui,
